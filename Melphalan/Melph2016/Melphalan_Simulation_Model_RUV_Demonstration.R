...
sumfuncy <- function(x) {
	...
	stat2 <- sd(x)
	stat3 <- length(x)
	stat4 <- stat1 + qnorm(0.025)*(stat2/sqrt(stat3))
	stat5 <- stat1 + qnorm(0.975)*(stat2/sqrt(stat3))
...
AMT <- 400	#Amount to be infused, mg
INFD <- 0.5	#Infusion duration, hours
RATE <- AMT/INFD	#Rate of infusion, mg/hour
...
#Make a data frame of one individual where ETAs == 0, this is the person representing the "PRED" (population predicted)
PRED.data <- data.frame(ID = 0,ETA1 = 0,ETA2 = 0,ETA3 = 0,ETA4 = 0,ETA5 = 0,ETA6 = 0,ETA7 = 0,ETA8 = 0,
...
IPRE.data <- data.frame(ID,ETA1,ETA2,ETA3,ETA4,ETA5,ETA6,ETA7,ETA8,
												CRCL,FFM,HCT,SLC7A5,BUN,ANCBASE,SEX,WBC,LNP53FOLD,GCSF)
#Bind the 2 data frames together, should contain n + 1 individual
input.data <- rbind(PRED.data,IPRE.data)					
...
DES <- function(T, A, PAR) {
	...
	##_STAT BLOCK_##
	################	
	dAdT[9] = 0	#STAT	Let me know how long the individual has spent in Grade 4 Neutropenia
	if (A[4] < 0.5) {dAdT[9] = 1} 
...
sim.data$PKIPRE <- sim.data$A1/sim.data$V1	#Melphalan concentration
sim.data$PDIPRE <- sim.data$C4	#ANC
...
sim.data$PKDV <- sim.data$PKIPRE*exp(EPS1)	#Melphalan concentration

EPS2 <- rnorm(n = length(sim.data$PDIPRE), mean = 0, sd = ERR2)
sim.data$PDDV <- sim.data$PDIPRE + W*EPS2	#ANC
...
statsPD <- ddply(PD.data, .(time), function(PD.data) sumfuncx(na.omit(PD.data$PDIPRE[PD.data$ID != 0])))	#Exclude the "PRED" individual I set up
...
#Blue line = Population predicted
#Blue ribbon = 95% Prediction intervals for simulated individual predictions
#Red line = Mean of simulated DV
#Red ribbon = 95% Confidence intervals of simulated DV
plotobj2 <- NULL
plotobj2 <- ggplot()
...
plotobj2 <- plotobj2 + geom_line(aes(x = time, y = PDIPRE), data = sim.data[sim.data$ID == 0,], colour = "blue")
...
#------------------------------------------------------------------------------------
#Pull out the duration in grade 4 neutropenia - it's in the last line for each individual
#Pull out the 24-hour interval where the individual entered Grade 4 Neutropenia
G4ND.function <- function(PD.data) {
	duration.data <- tail(PD.data,1)	#Find the duration spent in Grade 4 neutropenia (it's in their last line)
	duration.data$EVENT <- 1
	ind.length <- length(PD.data$S9[PD.data$S9 != 0])	#Find the length of time-points where S9 isn't 0
	#People with ind.length > 0, spent time in Grade 4 neutropenia
	if (ind.length > 0) entry.data <- PD.data[PD.data$S9 == min(PD.data$S9[PD.data$S9 != 0]),]
	#People with ind.length == 0, did not spend time in Grade 4 neutropenia
	if (ind.length == 0) entry.data <- head(PD.data,1)
	entry.data$EVENT <- 2
	ind.data <- rbind(entry.data,duration.data)
}
G4ND.data <- ddply(PD.data, .(ID), G4ND.function)

#Population predicted
G4ND_POP <- G4ND.data$S9[G4ND.data$ID == 0 & G4ND.data$EVENT == 1]
G4ND_POP <- round(G4ND_POP)	#Population predicted duration spent in Grade 4 Neutropenia

G4N_ENTRY_POP <- G4ND.data$time[G4ND.data$ID == 0 & G4ND.data$EVENT == 2] - G4ND.data$S9[G4ND.data$ID == 0 & G4ND.data$EVENT == 2]
G4N_ENTRY_POP <- round(G4N_ENTRY_POP)	#Population predicted entry in Grade 4 Neutropenia

G4N_EXIT_POP <- G4N_ENTRY_POP + G4ND_POP
G4N_EXIT_POP <- round(G4N_EXIT_POP)	#Population predicted exit from Grade 4 Neutropenia

#95% Prediction Intervals - based on individual predicted
G4ND_IND <- G4ND.data$S9[G4ND.data$ID > 0 & G4ND.data$EVENT == 1]
G4ND_LO95 <- round(quantile(G4ND.data$S9[G4ND.data$ID > 0 & G4ND.data$EVENT == 1], probs = 0.025))
G4ND_HI95 <- round(quantile(G4ND.data$S9[G4ND.data$ID > 0 & G4ND.data$EVENT == 1], probs = 0.975))
G4ND_INT <- paste("(",G4ND_LO95," - ",G4ND_HI95,")",sep="")	#95% Prediction intervals for duration spent in Grade 4 Neutropenia

G4N_ENTRY_IND <- G4ND.data$time[G4ND.data$ID > 0 & G4ND.data$EVENT == 2] - G4ND.data$S9[G4ND.data$ID > 0 & G4ND.data$EVENT == 2]
G4N_ENTRY_LO95 <- round(quantile(G4N_ENTRY_IND[G4N_ENTRY_IND > 0], probs = 0.025))	#Ignore those who never went into grade 4 neutropenia
G4N_ENTRY_HI95 <- round(quantile(G4N_ENTRY_IND[G4N_ENTRY_IND > 0], probs = 0.975))	#Ignore those who never went into grade 4 neutropenia
G4N_ENTRY_INT <- paste("(",G4N_ENTRY_LO95," - ",G4N_ENTRY_HI95,")",sep="")	#95% Prediction intervals for entry into Grade 4 Neutropenia

G4N_EXIT_IND <- G4N_ENTRY_IND + G4ND_IND
G4N_EXIT_LO95 <- round(quantile(G4N_EXIT_IND[G4N_EXIT_IND > 0], probs = 0.025))	#Ignore those who never went into grade 4 neutropenia
G4N_EXIT_HI95 <- round(quantile(G4N_EXIT_IND[G4N_EXIT_IND > 0], probs = 0.975))	#Ignore those who never went into grade 4 neutropenia
G4N_EXIT_INT <- paste("(",G4N_EXIT_LO95," - ",G4N_EXIT_HI95,")",sep="")	#95% Prediction intervals for exit out of Grade 4 Neutropenia

#Number of individuals who did not enter Grade 4 Neutropenia
NO_G4N <- length(G4ND.data$ID[G4ND.data$EVENT == 2 & G4ND.data$S9 == 0])
NO_G4N <- paste(NO_G4N/n*100,"% of individuals did not experience Grade 4 Neutropenia",sep="")

#Summary
G4ND <- paste("Duration in Grade 4 Neutropenia:",G4ND_POP,G4ND_INT,"hours",sep=" ")
G4N_ENTRY <- paste("Entry into Grade 4 Neutropenia:",G4N_ENTRY_POP,G4N_ENTRY_INT,"hours since melphalan dose",sep=" ")
G4N_EXIT <- paste("Exit from Grade 4 Neutropenia:",G4N_EXIT_POP,G4N_EXIT_INT,"hours since melphalan dose",sep=" ")
G4ND
G4N_ENTRY
G4N_EXIT
NO_G4N
