...
$MODEL 
 ...
 COMP=(G4ND120) ;COMPARTMENT FOR COLLECTING TIME SPENT IN GRADE 4 NEUTROPENIA IN THE FIRST 120 HOURS
 COMP=(G4ND240) ;COMPARTMENT FOR COLLECTING TIME SPENT IN GRADE 4 NEUTROPENIA IN THE SECOND 120 HOURS
 COMP=(G4ND360) ;COMPARTMENT FOR COLLECTING TIME SPENT IN GRADE 4 NEUTROPENIA IN THE THIRD 120 HOURS
...
$PK
 ...
 TVSLOPE=THETA(11)*(1+GCSF*THETA(21))*(1+SEX*THETA(23))*((HCT/32.50)**THETA(24)) 
...
$DES
 ...
 IF(TIME.GE.0.AND.TIME.LT.120) THEN
  DADT(9)=0
  IF(A(4).LT.0.5) DADT(9)=1
 ENDIF
 
 IF(TIME.GE.120.AND.TIME.LT.240) THEN
  DADT(10)=0
  IF(A(4).LT.0.5) DADT(10)=1
 ENDIF
 
 IF(TIME.GE.240.AND.TIME.LT.360) THEN
  DADT(11)=0
  IF(A(4).LT.0.5) DADT(11)=1
 ENDIF

$ERROR 
 G4ND_120=A(9)
 IF(TIME.GE.120.AND.TIME.LT.240) G4ND_240=A(10)
 IF(TIME.GE.240.AND.TIME.LT.360) G4ND_360=A(11)
...
$TABLE ID TIME AMT MDV DVID CMT Y IPRED PRED
G4ND_120 G4ND_240 G4ND_360
WT CRCL FFM HCT GCSF SLC7A5 BUN ANCBASE SEX WBC LNP53FOLD
TVSLOPE
NOPRINT ONEHEADER FILE=pkpd_final_sim_interval.fit 
